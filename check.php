...
check_row('Allen image cache directory', is_dir(app_config()['cache_dir']) || is_writable(dirname(app_config()['cache_dir'])), app_config()['cache_dir']);
...
foreach (array('diurnal', 'dv', 'supplemental', 'rostral_caudal') as $domain) {
    ...
    if ($ok && extension_loaded('pdo_sqlite')) {
        try {
            ...
            if ($domain === 'rostral_caudal') {
                $hasRc = db_one($pdo, "SELECT name FROM sqlite_master WHERE type='table' AND name='rc_genes' LIMIT 1") !== null;
                $detail .= '; rc_tables=' . ($hasRc ? 'installed' : 'not installed');
            }
